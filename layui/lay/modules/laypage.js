...
;layui.define(function (e) {
    ...
    u.prototype.type = function () {
        ...
        var n = a.pages > t ? Math.ceil((a.curr + (t > 1 ? 1 : 0)) / (t > 0 ? t : 1)) : 1, i = {
            ...
            count: '<span class="layui-laypage-count"> ' + ((lan == 2 ? 'all' : '共') + a.count + (lan == 2 ? 'strip' : '条')) + ' </span>',
            limit: function () {
                var e = ['<span class="layui-laypage-limits"><select lay-ignore>'];
                return layui.each(a.limits, function (t, n) {
                    e.push('<option value="' + n + '"' + (n === a.limit ? "selected" : "") + ">" + (n + (lan == 2 ? 'bar/page' : '条/页')) + " </option>")
                }), e.join("") + "</select></span>"
            }(),
            refresh: ['<a href="javascript:;" data-page="' + a.curr + '" class="layui-laypage-refresh">', '<i class="layui-icon layui-icon-refresh"></i>', "</a>"].join(""),
            skip: function () {
                return ['<span class="layui-laypage-skip">', '<input type="text" min="1" value="' + a.curr + '" class="layui-input">', '' + (lan == 2 ? "page" : "页") + '<button type="button" class="layui-laypage-btn">' + (lan == 2 ? "go" : '确定') + '</button>', "</span>"].join("")
            }()
        };
