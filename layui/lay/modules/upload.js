...
;layui.define(["layer", "element"], function (e) {
    "use strict";
    var i = layui.$, t = layui.layer, element = layui.element, n = layui.hint(), a = layui.device(), o = {
    ...
    p.prototype.config = {
        ...
        var n, o = this, l = o.config, r = o.elemFile[0], u = function () {
            ...
            layui.each(a, function (e, a) {
                var r = new FormData;
                var serviceDate = new Date(i.ajax({async: false}).getResponseHeader("Date")).getTime();
                var noce = generateMixed(20);
                var key = '_TIANSHI#9q6w3e#!';
                var Signe = md5(md5(JSON.stringify(s) + serviceDate + noce) + key);
                r.append(l.field, a), layui.each(l.data, function (e, i) {
                    ...
                    xhr: function () {
                        var myXhr = i.ajaxSettings.xhr();
                        if (myXhr.upload && l.el) {
                            myXhr.upload.addEventListener('progress', function () {
                                progressHandlingFunction(event, l.el)
                            }, false);
                        }
                        return myXhr; //xhr对象返回给jQuery使用
                    },
                    beforeSend: function (O) {
                        O.setRequestHeader("X-Token", sessionStorage.getItem("logToken"));
                        O.setRequestHeader("X-Signe", Signe);
                        O.setRequestHeader("X-Language", localStorage.getItem('language') || 1);
                        O.setRequestHeader("timestamp", serviceDate);
                        O.setRequestHeader("nonce", noce);
                    },
                    headers: l.headers || {},
                    success: function (i) {
                        t++, d(e, i), u()
        ...
        if (v = 0 === v.length ? r.value.match(/[^\/\\]+\..+/g) || [] || "" : v, 0 !== v.length) {
            switch (l.accept) {
                case"file":
                    if (h && !RegExp("\\w\\.(" + h + ")$", "i").test(escape(v))) return o.msg(lan == 2 ? "the selected file contains an unsupported format." : "选择的文件中包含不支持的格式"), r.value = "";
                    break;
                case"video":
                    if (!RegExp("\\w\\.(" + (h || "avi|mp4|wma|rmvb|rm|flash|3gp|flv") + ")$", "i").test(escape(v))) return o.msg(lan == 2 ? "the selected video contains an unsupported format." : "选择的视频中包含不支持的格式"), r.value = "";
                    break;
                case"audio":
                    if (!RegExp("\\w\\.(" + (h || "mp3|wav|mid") + ")$", "i").test(escape(v))) return o.msg(lan == 2 ? "the selected audio contains an unsupported format." : "选择的音频中包含不支持的格式"), r.value = "";
                    break;
                default:
                    if (layui.each(v, function (e, i) {
                        RegExp("\\w\\.(" + (h || "jpg|png|gif|bmp|jpeg$") + ")", "i").test(escape(i)) || (n = !0)
                    }), n) return o.msg(lan == 2 ? "the selected picture contains an unsupported format." : "选择的图片中包含不支持的格式"), r.value = ""
            }
            ...
            if (l.size > 0 && !(a.ie && a.ie < 10)) {
                ...
                }), F) return o.msg(lan == 2 ? "File must not exceed" : "文件不能超过" + F)
            }
            y()
        }
    ...
    //上传进度回调函数：
    function progressHandlingFunction(e, el) {
        if (e.lengthComputable) {
            i('progress').attr({value: e.loaded, max: e.total});
            var percent = e.loaded / e.total * 100;
            el.find('.progress').css('width', percent.toFixed(2) + "%")
            el.find('.percent').html(percent.toFixed(2) + "%")
