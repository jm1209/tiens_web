...
//全局的ajax调用
// var interfaceUrl = "http://172.16.0.35:8080/tiens-console/";//王杰
//var interfaceUrl = "http://172.16.0.22:8080/tiens-console/";// 王俊宇
// var interfaceUrl = "http://172.16.0.241:8081/tiens-console/";//内网测试
var interfaceUrl = "http://221.239.72.121:8080/tiens-console/";//外网测试
    ...
    $.ajax({
        ...
        success: function (d) {
            if (d.code == "0") {
                ...
                layer.msg(d.msg)
            } else if (d.code == "30007") {
                layer.msg(d.msg)
            } else if (d.code == "20001") {
                layer.msg(d.msg)
            } else if (d.code == "30002") {
                layer.msg(LOGINFIRST || "您还没登录，请先登录！");
                setTimeout(function () {
                    parent.goLogin();
                }, 500)
            }
...
//封装全局数据表格渲染
var tableIns;
var tableInit = function (table, url, colArr) {
...
layui.use(['table', 'jquery', 'layer'], function () {
    var $ = layui.jquery, table = layui.table, layer = parent.layer === undefined ? layui.layer : top.layer;
    ...
    $('.memAdd').click(function () {
        var index = layer.open({
            ...
            yes: function (index, layero) {
                ...
                if (name == '') {
                    layer.msg(lan == 2 ? 'the label is empty, please input.' : '标签名称为空，请输入');
                    return;
                }
                if ($('.tag').length > 5) {
                    layer.msg(MORETAG || '标签不可过多');
                    return;
                }
                for (var i = 0; i < $('.tag').length; i++) {
                    if ($('.tag').eq(i).find('.tagText').html() === name && $('.tag').length >= 1) {
                        layer.msg(EXSISTTAG || '该标签已存在');
                        return;
                    }
                }
                addTag(name, color);
